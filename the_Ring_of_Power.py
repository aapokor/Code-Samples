...
def main():
    ...
    device, roi_objects, hierarchy, kept_mask, obj_area = pcv.roi_objects(img, 'partial', roi_contour, roi_hierarchy,
                                                                          objects, obj_hierarchy, device,
                                                                          debug=args.debug)
    ...
    if roi_objects > 0:
        ...
        device, shape_header, shape_data, shape_img = pcv.analyze_object(img=img, imgname=args.image, obj=plant_contour,
                                                                         ...
                                                                         debug=args.debug, filename=outfile + ".png")

        # In[123]:
		# Set line position by group
        if "TM015" in args.image:
            line_position = 380
        elif "TM016" in args.image:
            line_position = 440
